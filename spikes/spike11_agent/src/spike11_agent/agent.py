...
def _planner_decide(
    ...
    # Keep state concise in the prompt
    ctx = json.dumps(
...
def fallback_pipeline(question: str, top_k: int = 8) -> dict[str, Any]:
    # 1) Find candidate episodes (metadata/content)
    # eps = episode_locator({"query": question, "limit": 5})
    # epids = [e["episode_id"] for e in eps.get("episodes", []) if e.get("episode_id")]

    # 2) RAG search corpus-wide (robust default)
    rag = rag_search(
        {
            "question": question,
            "episode_id": None,
    ...
    # 3) Build a compact timeline summary
    segs = [
        {
...
def _format_history(hist: list[dict[str, str]] | None, max_chars: int = 1200) -> str:
    """
    Turn [{"role":"user"|"assistant","content": "..."}] into a compact string.
    Trim to avoid blowing the context window.
    """
    if not hist:
        return ""
    lines = []
    for h in hist[-8:]:  # last 8 turns max
...
def run_deep_agent(
    ...
    chat_history: list[dict[str, str]] | None = None,  # <<< NEW
) -> dict[str, Any]:
    scratch: list[dict[str, Any]] = []
    ...
    with start_span(
        ...
        for step in range(step_cap):
            ...
            # If the planner produced a final answer
            ...
            # If planner failed to produce valid JSON → fallback
            ...
            # Normalize rag_search args with sensible defaults
        ...
        # Step cap reached → fallback then finalize
        fb = fallback_pipeline(question=question, top_k=top_k)
        try:
            validate(fb, TASK_OUT_SCHEMA)
        except ValidationError:
