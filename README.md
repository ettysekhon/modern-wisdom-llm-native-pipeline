...
####  Next Steps — Toward Full RAG Pipeline

| Stage                          | From Spike | Purpose                                          |
| ------------------------------ | ---------- | ------------------------------------------------ |
| **Chunk & Embed All Episodes** | 3 + 4      | Loop over all transcripts and persist embeddings |
| **Bulk Qdrant Upsert**         | 5          | Batch insert via CLI or Python loop              |
| **Retriever & Reranker**       | 6 (next)   | Query `mw_chunks_live` for semantic context      |
| **Context Builder**            | 7 (future) | Integrate with LLM for answer generation         |
